...
Mdl = liblineartrain(double(Ytr), sparse(double(Xtr)), 's', 1);
...
for k=1:length(val_file)
    ...
    [patches,temp_bbox] = sw_detect_face(img,window_size,1.0, 1.0,32);    
    ...
    for p=1:length(patches)
        ...
        for j = 1:size(patches{p},3)
            ...
            %face_img = vl_lbp(face_img, 8);
            face_img = vl_hog(face_img, 8);
            Xte = [Xte face_img(:)];
            bbox_ms = [bbox_ms;temp_bbox{p}(j,:)];
        end
        % Get the positive probability for proposed faces
        Xte = Xte';    
        [~,~,d] = liblinearpredict(ones(size(Xte,1),1),sparse(double(Xte)),Mdl);
        prob2 = 1./(1+exp(-d));
        prob = [1-prob2, prob2];
    ...
    %     prob2 = score(:,2);
        ...
        threshold = 0.5;
        threshold_bbox=[threshold_bbox; bbox_ms(prob2>threshold,:)];
        prob3=[prob3;prob2(prob2>threshold,:)];
    ...
    [selectedBbox,selectedScore] = selectStrongestBbox(threshold_bbox,prob3,'OverlapThreshold',0.1, 'RatioType','Union');
...
for k=1:length(val_file2)
    ...
    for j = 1:length(imset(k).Count)
        ...
        [patches,temp_bbox] = sw_detect_face(img,window_size,1, 1, 32);    
        Xte=[];
        bbox_ms = [];
        ...
        Xte = Xte';
        [l,~,d] = liblinearpredict(ones(size(Xte,1),1),sparse(double(Xte)),Mdl);
        prob2 = 1./(1+exp(-d));
        prob = [1-prob2, prob2];
