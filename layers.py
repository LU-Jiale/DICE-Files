...
from mlp.im2col import im2col_indices, col2im_indices, max_pool
...
class BatchNormalizationLayer(StochasticLayerWithParameters):
    ...
    def fprop(self, inputs, stochastic=True):
        ...
        input_normalized = ((inputs - np.mean(inputs.T, 1))/np.sqrt(np.var(inputs.T, 1) + self.epsilon))
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        Returns:
            ...
            (batch_size, input_dim).
        """
        std = np.sqrt(np.var(inputs.T, 1) + self.epsilon)
        mean = np.mean(inputs.T, 1)
        batch_size = len(inputs)
        e_u_normal = grads_wrt_outputs*self.gamma
        e_var = np.sum((e_u_normal*(inputs - mean)*(-0.5*std**(-3))).T, 1)
        e_mean = np.sum((-e_u_normal/std).T, 1) + e_var * np.sum(-2*(inputs - mean).T, 1)/batch_size
        e_u = e_u_normal/std + e_var*2*(inputs-mean)/batch_size + e_mean/batch_size
        return e_u
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        """
        input_normalized = ((inputs - np.mean(inputs.T, 1))/np.sqrt(np.var(inputs.T, 1) + self.epsilon))
        e_gamma = np.sum((grads_wrt_outputs*input_normalized).T,1)
        e_beta = np.sum(grads_wrt_outputs.T,1)
        return [e_gamma, e_beta]
...
class MaxPoolingLayer(Layer):
    ...
    def __init__(self, batch_size, num_input_channels,
        ...
        self.output_dim_1 = int(np.floor(input_dim_1 / kernel_dim_1))
        self.output_dim_2 = int(np.floor(input_dim_2 / kernel_dim_2))
        self.kernels_shape = (self.batch_size, num_input_channels, input_dim_1, input_dim_2)
        self.inputs_shape = [
    ...
    def fprop(self, inputs):
        ...
        Returns:
            outputs: Array of layer outputs of shape (batch_size, output_dim).
        """
        [ouput, self.kernels] = max_pool(inputs, self.kernel_dim_1, self.kernel_dim_2, self.kernel_dim_1)

        return  ouput
        # [output, self.kernels] = conv_4d.maxP_fprop(inputs, self.kernels, self.output_dim_1,
        #                                             self.output_dim_2, self.kernel_dim_1, self.kernel_dim_2)
        # return output
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        Returns:
            ...
            (batch_size, input_dim).
        """
        gradient = np.repeat(grads_wrt_outputs,self.kernel_dim_2).reshape(self.batch_size,self.num_input_channels,
                                                                          self.output_dim_1,self.input_dim_2)

        gradient = np.repeat(gradient.transpose(0,1,3,2), self.kernel_dim_1).\
            reshape(self.batch_size,self.num_input_channels, self.input_dim_2,self.input_dim_1).transpose([0,1,3,2])
        return gradient*self.kernels
        # return conv_4d.maxP_bprop(inputs, grads_wrt_outputs, self.kernels,
        #                           self.output_dim_1, self.output_dim_2, self.kernel_dim_1, self.kernel_dim_2)
    ...
    def __repr__(self):
        return 'Maxpooling (input_dim={0}, output_dim={1})'.format(
            self.input_dim, self.output_dim)
...
class ConvolutionalLayer(LayerWithParameters):
    ...
    def __init__(self, num_input_channels, num_output_channels,
        ...
        self.output_shape = [
            None, num_output_channels, input_dim_1 - kernel_dim_1 + 1, input_dim_2 - kernel_dim_2 + 1
        ]
    ...
    def fprop(self, inputs):
        ...
        cols = im2col_indices(inputs, self.kernel_dim_1, self.kernel_dim_2, padding=0)
        new_kernal = self.kernels[:, :, ::-1, ::-1]
        new_kernal = new_kernal.reshape(self.num_output_channels, -1)
        new_biases = self.biases.reshape(-1, 1)
        out = np.dot(new_kernal, cols) + new_biases
        self.cache = col2im_indices(out, self.output_shape, self.output_shape[2], self.output_shape[3], padding=0)
        # self.cache = conv_4d.cnn_fprop(inputs, self.kernels, self.biases, self.num_input_channels)
        return self.cache
    ...
    def bprop(self, inputs, outputs, grads_wrt_outputs):
        ...
        self.inputs_shape[0] = inputs.shape[0]
        cols = im2col_indices(grads_wrt_outputs, self.kernel_dim_1, self.kernel_dim_2, padding=self.kernel_dim_1-1)
        new_kernal = self.kernels.transpose(1,0,2,3).reshape(self.num_input_channels, -1)
        out = np.dot(new_kernal, cols)
        output = col2im_indices(out, self.inputs_shape, self.input_dim_1, self.input_dim_2, padding=0)
        return output

        # return conv_4d.cnn_bprop(inputs, grads_wrt_outputs, self.kernels, self.biases, self.num_input_channels)
    ...
    def grads_wrt_params(self, inputs, grads_wrt_outputs):
        ...
        """
        self.inputs_shape[0] = inputs.shape[0]
        cols = im2col_indices(grads_wrt_outputs.transpose(1, 0, 2, 3), self.input_dim_1, self.input_dim_2, padding=self.kernel_dim_1 - 1)
        new_inputs = inputs.transpose(1, 0, 2, 3).reshape(self.num_input_channels, -1)
        out = np.dot(new_inputs, cols)
        grad_weight = col2im_indices(out, self.kernels_shape, self.kernel_dim_1, self.kernel_dim_2, padding=0)
        grad_biase = np.sum(np.sum(np.sum(grads_wrt_outputs, axis = 3), axis = 2), axis = 0)

        output = [grad_weight, grad_biase]
        return output

        # return conv_4d.cnn_grads(inputs, grads_wrt_outputs, self.kernels, self.biases)
...
class ReshapeLayer(Layer):
    ...
    def __init__(self, output_shape=None):
        ...
        Args:
            output_shape: Tuple specify   ing shape each input in batch should
